...
DEFAULT_USER_DATA = {
    "abhishek": {"password": "abhi123", "team": "♠️ ABHISHEK BHIYA"},
    "sidhu": {"password": "sidhu123", "team": "🦁 SIDHU"},
    "admin": {"password": "1234", "team": "👑 ADMIN"}
}
...
def load_db():
    default_db = {
        ...
        "rtm_cards": {v["team"]: True for k, v in DEFAULT_USER_DATA.items() if k != "admin"}
    }
...
USER_DATA = db["users"]
players = db["players"]
teams = [v["team"] for k, v in USER_DATA.items() if k != "admin"]
...
if not st.session_state['logged_in']:
    ...
    with col2:
        with st.form("login"):
            st.markdown("### 🧑‍✈️ Captain / Admin Login")
...
<span style="color:#00FA9A;">{expensive['Final Points']} pts</span>
    ...
    if len(teams) > 0:
        fig = px.pie(values=list(purses.values()), names=list(purses.keys()), hole=0.6, color_discrete_sequence=['#FFD700', '#FF4500', '#00FA9A', '#1E90FF'])
...
""", unsafe_allow_html=True)
    ...
    if st.session_state['user_role'] not in ["admin", "viewer"]:
    ...
    with st.expander("🛠️ Admin Controls", expanded=True):
        ...
        with tm2:
            with st.form("remove_team"):
                ...
                rem_options = [k for k in db["users"].keys() if k != "admin"]
                if rem_options:
