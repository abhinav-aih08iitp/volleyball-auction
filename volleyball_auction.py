...
.player-card {{ background: rgba(20, 20, 20, 0.9); padding: 40px; border-radius: 25px; border: 2px solid #FFD700; backdrop-filter: blur(8px); box-shadow: 0 10px 40px rgba(255, 215, 0, 0.3); text-align: center; }}
.photo-frame {{ width: 250px; height: 250px; border-radius: 50%; border: 6px solid #FFD700; box-shadow: 0 0 30px #FFD700; object-fit: cover; margin: 0 auto 25px auto; display: block; background: #111; }}
.category-badge {{ background-color: #FF4500; color: white; padding: 8px 20px; border-radius: 12px; font-weight: bold; font-size: 20px; text-transform: uppercase; letter-spacing: 1px; }}
...
if db["player_index"] >= len(players):
    ...
    with col2:
        ...
        p_img = get_base64(current_player.get("Photo", "default.jpg"))
        if p_img: st.markdown(f'<img src="data:image/jpeg;base64,{p_img}" class="photo-frame">', unsafe_allow_html=True)
        else: st.markdown("<div class='photo-frame' style='display:flex; align-items:center; justify-content:center; font-size:100px;'>🏐</div>", unsafe_allow_html=True)
        
        st.markdown(f"""
            <span class="category-badge">{current_player["Role"]}</span>
            <h1 style="color:#FFD700; margin-top:20px; font-size:50px; text-shadow: 2px 2px #000;">{current_player["Name"]}</h1>
            <h3 style="color:#00FA9A; font-weight:bold;">BASE PRICE: {actual_base} PTS</h3>
        """, unsafe_allow_html=True)
        ...
        with bc3:
            ...
            if st.button("🃏 RTM CARD", disabled=not can_rtm, use_container_width=True):
                with db_lock:
                    ...
                    fdb.update({"winner_name":f"RTM USED BY {me}!", "last_sold_trigger":True, "player_index":fdb["player_index"]+1, "current_bid":0, "current_team":"None"})
                    save_db(fdb)
                st.rerun()

# --- 8. MASTERJI CONTROLS ---
if st.session_state['user_role'] == "Masterji":
    with st.expander("🛠️ Masterji Control Panel", expanded=True):
        ...
        if avail:
            call = st.selectbox("Select Player to Stage", avail)
        ...
        with tm1:
            with st.form("add_team"):
                n_id, n_pass, n_name = st.text_input("New ID"), st.text_input("New Pass"), st.text_input("Team Name & Emoji")
        ...
        with tm2:
            with st.form("rem_team"):
                r_id = st.selectbox("Remove Team", [k for k in db["users"] if k != "Masterji"])
